...
export const OrdersSection: React.FC<OrdersSectionProps> = ({
  ...
  const [filterStatus, setFilterStatus] = useState<
    "Pendiente" | "Aprobado" | "Rechazado" | "Todas"
  >("Pendiente");
  ...
  const approvedCount = sortedOrders.filter(
    (o) => o.status === "Aprobado",
  ).length;
  const rejectedCount = sortedOrders.filter(
  ...
  const handleApprove = async (order: Order) => {
    ...
    const paymentTypeToUse =
      selectedPaymentTypes[order.id] || order.paymentType || "contado";
  ...
  const handleDelete = async (order: Order) => {
    if (order.status === "Pendiente") {
      alert(
        "Las solicitudes en estado Pendiente no pueden ser eliminadas. Primero debes aprobarla o rechazarla.",
      );
      return;
    }
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {filterStatus === "Pendiente" &&
                "Mostrando únicamente solicitudes pendientes por aprobar"}
              {filterStatus === "Aprobado" &&
                "Mostrando solicitudes aprobadas."}
              {filterStatus === "Rechazado" &&
                "Mostrando solicitudes rechazadas."}
              {filterStatus === "Todas" &&
                "Mostrando el historial completo de solicitudes recibidas."}
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm max-w-md mx-auto my-6">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Cuando los clientes realicen compras en la tienda, aparecerán aquí
            inmediatamente.
          </p>
        </div>
      ) : (
                  ...
                  {isPending && stockInfo.hasIssue && (
                    <div className="p-2.5 bg-rose-50 border border-rose-200 rounded-xl text-rose-900 text-[11px] font-bold space-y-0.5">
                      <div className="flex items-center gap-1 text-rose-700 font-extrabold">
                        ...
                        <span>
                          Solo queda(n) {stockInfo.issues[0]?.available ?? 0}{" "}
                          disponible(s)
                        </span>
                      </div>
                      <p className="text-[10px] text-rose-800 font-medium">
                        No se puede aprobar (se solicitaron{" "}
                        {stockInfo.issues[0]?.requested} unidades). Solo
                        disponible rechazar.
                      </p>
                    </div>
                  )}
      ...
      {selectedOrderForModal &&
        (() => {
          const modalStockInfo = getInsufficientStockInfo(
            selectedOrderForModal,
          );
                  ...
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-xs font-extrabold text-emerald-400 bg-slate-800 px-2.5 py-1 rounded-md border border-slate-700">
                      ...
                      {selectedOrderForModal.orderNumber ||
                        selectedOrderForModal.id.slice(0, 8)}
                    </span>
                ...
                <div className="p-6 space-y-5 overflow-y-auto flex-1">
                    ...
                    modalStockInfo.hasIssue && (
                      <div className="bg-rose-50 border border-rose-200 p-4 rounded-2xl space-y-2 text-rose-900 animate-in fade-in">
                        <div className="font-extrabold text-xs flex items-center gap-2 text-rose-700">
                          ...
                          <span>
                            Stock insuficiente en inventario para aprobar esta
                            solicitud
                          </span>
                        </div>
                        <div className="space-y-1 text-xs">
                          {modalStockInfo.issues.map((iss, i) => (
                            <p key={i} className="font-semibold text-rose-800">
                              • <strong>{iss.productName}</strong>: Solicitadas{" "}
                              <strong>{iss.requested}</strong> unidad(es) |
                              Stock disponible:{" "}
                              <span className="bg-rose-200 text-rose-950 px-2 py-0.5 rounded font-black">
                                {iss.available}
                              </span>
                            </p>
                          ))}
                        ...
                        <p className="text-[11px] text-rose-700 font-medium pt-1 border-t border-rose-200/60">
                          Debido a que el stock actual es menor al solicitado,
                          no se puede aprobar esta solicitud. Solo está
                          disponible la opción de <strong>Rechazar</strong>.
                        </p>
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                      ...
                      <select
                        value={
                          selectedPaymentTypes[selectedOrderForModal.id] ||
                      ...
                      >
                        <option value="contado">
                          De Contado (C$ {calculatedModalTotal.toFixed(2)})
                        </option>
                        <option value="cuotas_2">
                          2 Cuotas Quincenales (2x C${" "}
                          {(calculatedModalTotal / 2).toFixed(2)})
                        </option>
                        <option value="cuotas_4">
                          4 Cuotas Semanales (4x C${" "}
                          {(calculatedModalTotal / 4).toFixed(2)})
                        </option>
                      </select>
                  ...
                  <div className="space-y-2">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2.5">
                              <span
                                className={`font-bold px-2.5 py-0.5 rounded-md ${isLowStk ? "bg-rose-100 text-rose-800 border border-rose-300" : "bg-emerald-100 text-emerald-700"}`}
                              >
                                {item.quantity}x
                              </span>
                              <div>
                                ...
                                {isLowStk && (
                                  <span className="block text-[10px] text-rose-600 font-bold">
                                    ⚠️ Quedan solo {currentStk} unidad(es) en
                                    inventario
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="flex justify-between items-center pt-2 px-1 text-sm font-extrabold text-slate-900">
                      ...
                      <span className="text-emerald-600 text-xl font-black">
                        C$ {calculatedModalTotal.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    {selectedOrderForModal.status === "Pendiente" ? (
                      modalStockInfo.hasIssue ? (
                        ...
                        >
                          ...
                          <span>
                            Rechazar Solicitud (Solo quedan{" "}
                            {modalStockInfo.issues[0]?.available ?? 0})
                          </span>
                        </button>
                      ) : (
                        <>
