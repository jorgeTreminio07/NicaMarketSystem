...
> = ({ orders, products = [], onRefresh, isLoading }) => {
  ...
  const [selectedCreditForModal, setSelectedCreditForModal] =
    useState<Order | null>(null);

  // Modal state for adding an abono
  const [selectedOrderForAbono, setSelectedOrderForAbono] =
    useState<Order | null>(null);
  ...
  const totalCollected = approvedOrders.reduce(
    (sum, o) => sum + (o.totalPaid || 0),
    0,
  );
  ...
  const handleOpenAbonoModal = (order: Order) => {
    ...
    if (order.creditStatus === "Pagado" || remainingBalance <= 0) {
      alert(
        "Esta solicitud ya ha sido pagada en su totalidad. No es posible registrar más abonos.",
      );
      return;
    }
    ...
    const nextUnpaidInstallment = order.paymentSchedule?.find(
      (s) => s.status !== "Pagado",
    );
    const defaultAmount = nextUnpaidInstallment
      ? nextUnpaidInstallment.expectedAmount
      : remainingBalance;
  ...
  const handleAddAbonoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        selectedCreditForModal.id === selectedOrderForAbono.id
      ) {
  ...
  const handleOpenEditAbonoModal = (
    order: Order,
    abono: { id: string; amount: number; note?: string },
  ) => {
    setEditingAbono({
      orderId: order.id,
      abonoId: abono.id,
  ...
  const handleEditAbonoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        selectedCreditForModal &&
        selectedCreditForModal.id === editingAbono.orderId
      ) {
        setSelectedCreditForModal(result.order);
      }
      ...
      const msg =
        err instanceof Error ? err.message : "Error al modificar el abono";
      setErrorMessage(msg);
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 mt-0.5">
              C$ {totalPortfolioValue.toFixed(2)}
            </h3>
            <p className="text-[10px] text-slate-500 font-medium mt-1">
              {approvedOrders.length} solicitudes aprobadas
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-700 font-bold mt-1">
              {totalPortfolioValue > 0
                ? Math.round((totalCollected / totalPortfolioValue) * 100)
                : 0}
              % recuperado
            </p>
          </div>
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-medium mt-1">
              {inProcessCount + overdueCount} cuentas por cobrar
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900">
              Cartera de Abonos y Cobros
            </h2>
            <p className="text-xs text-slate-500">
              Control general de solicitudes aprobadas, abonos recibidos, cuotas
              programadas y estados de saldo.
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm max-w-md mx-auto my-6">
          ...
          <h3 className="text-base font-bold text-slate-900">
            No hay solicitudes registradas en cartera
          </h3>
          <p className="text-xs text-slate-500 mt-1">
            Las solicitudes aprobadas aparecerán aquí automáticamente para
            gestionar sus abonos.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
                    ...
                    <div>
                      ...
                      {isEnMora && (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-rose-100 text-rose-800 border border-rose-300 animate-pulse">
                          <AlertTriangle className="w-3 h-3 text-rose-600" /> EN
                          MORA
                        </span>
                      )}
                      {!isPagado && !isEnMora && (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-extrabold bg-amber-100 text-amber-900 border border-amber-300">
                          <Clock className="w-3 h-3 text-amber-700" /> EN
                          PROCESO
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 pt-1 text-[11px] text-center">
                      ...
                      <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                        ...
                        <span
                          className={`font-extrabold ${remainingBalance > 0 ? "text-rose-600" : "text-emerald-600"}`}
                        >
          ...
          {visibleCount < filteredOrders.length && (
            <div
              ref={loadMoreRef}
              className="py-4 text-center text-xs font-semibold text-slate-400 flex items-center justify-center gap-2"
            >
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                      ...
                      <div>
                        ...
                        {!isPagado && !isEnMora && (
                          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-extrabold bg-amber-100 text-amber-900 border border-amber-300">
                            <Clock className="w-3.5 h-3.5 text-amber-700" /> EN
                            PROCESO DE PAGO
                          </span>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                      ...
                      <div className="bg-white p-3 rounded-xl border border-slate-200 text-center">
                        ...
                        <span
                          className={`text-base font-black ${remainingBalance > 0 ? "text-rose-600" : "text-emerald-600"}`}
                        >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    ...
                    <div className="space-y-2">
                      <h4 className="text-[11px] font-bold uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                        ...
                        <span>
                          Fechas de Pago ({order.paymentSchedule?.length || 0}{" "}
                          cuotas):
                        </span>
                      </h4>

                      {!order.paymentSchedule ||
                      order.paymentSchedule.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      <h4 className="text-[11px] font-bold uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
                        ...
                        <span>
                          Historial de Abonos (
                          {order.paymentsHistory?.length || 0}):
                        </span>
                      </h4>
                      ...
                      order.paymentsHistory.length === 0 ? (
                        ...
                        <div className="bg-slate-50 rounded-2xl border border-slate-200 divide-y divide-slate-200 max-h-60 overflow-y-auto">
                          {order.paymentsHistory.map((abono) => (
                            <div
                              key={abono.id}
                              className="p-3 text-xs flex items-center justify-between bg-white hover:bg-slate-50 transition-colors"
                            >
                              <div>
                                ...
                                <span className="text-[10px] text-slate-400 font-medium">
                                  {new Date(abono.date).toLocaleString(
                                    "es-NI",
                              ...
                              <div className="flex items-center gap-1.5">
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleOpenEditAbonoModal(order, abono)
                                  }
                                  title="Editar abono"
                ...
                <div className="bg-slate-50 p-4 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0">
                  ...
                  {isPagado ? (
                    <div className="text-xs font-bold text-emerald-800 bg-emerald-100 px-4 py-2 rounded-xl border border-emerald-300 flex items-center gap-1.5">
                      ...
                      <span>
                        Solicitud Pagada en su totalidad. No requiere más
                        cuotas.
                      </span>
                    </div>
                  ) : (
            ...
            <form onSubmit={handleAddAbonoSubmit} className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-xs space-y-1">
                ...
                <div className="flex justify-between text-slate-600 font-medium">
                  ...
                  <span>
                    {selectedOrderForAbono.orderNumber ||
                      selectedOrderForAbono.id.slice(0, 8)}
                  </span>
                </div>
                <div className="flex justify-between font-extrabold pt-1 border-t border-slate-200 text-slate-900">
                  ...
                  <span className="text-emerald-600">
                    C${" "}
                    {(
                      selectedOrderForAbono.total -
                      (selectedOrderForAbono.totalPaid || 0)
                    ).toFixed(2)}
                  </span>
                </div>
              </div>
            ...
            <form onSubmit={handleEditAbonoSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingAbono({ ...editingAbono, note: e.target.value })
                  }
